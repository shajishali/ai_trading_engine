...
@shared_task
def generate_enhanced_signals_task():
    """Generate enhanced signals using the enhanced signal generation service - runs every 1 hour"""
    try:
        logger.info("=" * 60)
        logger.info("Starting enhanced signal generation task")
        logger.info("=" * 60)
        
        # Generate signals using enhanced service
        ...
        # Log best signals
        if result.get('best_signals'):
            logger.info("Best signals generated:")
            for i, signal in enumerate(result['best_signals'][:5], 1):  # Show top 5
                logger.info(
        ...
        logger.error(f"Error in enhanced signal generation task: {e}", exc_info=True)
        return {
            'success': False,
            'error': str(e)
        }